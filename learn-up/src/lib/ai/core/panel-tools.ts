...
import { getAppRoute, isValidInternalRoute } from "./route-registry";
...
export const panelTools=[
  {
    name:"read_professor_graph", description:"Lee el Grafo de Conocimiento real del usuario para el panel Profesor IA.", requiresConfirmation:false, externalEffect:false, supportsAutopilot:true,
  ...
  {
    name:"read_professor_docs", description:"Lee los documentos indexados reales del usuario para el panel Docs de Profesor IA.", requiresConfirmation:false, externalEffect:false, supportsAutopilot:true,
    schema:z.object({}),
    execute:async()=>{
      const {supabase,user}=await authContext();
  ...
  {
    name:"log_advisor_mood", description:"Registra un estado de ánimo en el Tracker de Ánimo; por tratarse de un dato sensible, requiere autorización incluso en piloto automático.", requiresConfirmation:true, externalEffect:true, supportsAutopilot:false,
  ...
  {
    name:"add_advisor_journal_entry", description:"Guarda una entrada del Diario Emocional; por tratarse de contenido sensible, requiere autorización.", requiresConfirmation:true, externalEffect:true, supportsAutopilot:false,
  ...
  },
  {
    ...
    execute:async({name,quantity,unit}:{name:string;quantity?:string|number;unit?:string})=>{
      const {supabase,user}=await authContext();
      const numericQuantity=quantity===undefined?null:Number(quantity);
      const {data,error}=await supabase.from("nutrition_shopping_items").insert({user_id:user.id,name,quantity:Number.isFinite(numericQuantity as number)?numericQuantity:null,unit:unit||null,checked:false}).select("id,name,quantity,unit,checked,created_at,updated_at").single();
      if(error) throw error;
