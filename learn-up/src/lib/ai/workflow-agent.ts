...
function extractSources(data: any) {
  ...
  const raw = [data.sources, data.results, data.pages, data.evidence]
    ...
    .flat()
    .map((item: any) => ({ title: item?.title || item?.name || item?.url, url: item?.url || item?.sourceUrl || item?.link, provider: item?.provider }))
...
function appendUsedSources(text: string, sources: Array<{ title?: string; url: string; provider?: string }>) {
  ...
  return `${text.trim()}\n\nFuentes consultadas:\n${unique.slice(0, 12).map((source) => `• ${source.title || source.url} — ${source.url}`).join("\n")}`.trim();
}
...
async function withTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
  ...
  try {
    return await Promise.race([
      promise,
      new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error(`Timeout en ${label} (${ms}ms).`)), ms); }),
    ]);
  } finally {
...
async function executeTool(action: ToolAction, options: WorkflowRunOptions, step: number) {
  ...
  try {
    ...
    if (!registered?.execute) {
      throw new Error(`La herramienta ${action.tool} no pertenece a las 10 skills activas o no tiene ejecutor real.`);
    }
    ...
    const result = await withTimeout(
      Promise.resolve(registered.execute(parsed.data, { userId: options.userId, sessionId: options.sessionId || undefined, currentRoute: options.currentRoute, mediaUrl: options.mediaUrl, mediaType: options.mediaType } as any)),
      MAX_TOOL_MS,
      action.tool,
    );
    const materialized = await materializeToolResult(result, action.tool, effectiveArgs);
    const normalized = { action: { ...action, args: effectiveArgs }, success: Boolean(materialized?.success), message: String(materialized?.message || (materialized?.success ? "Completado" : materialized?.error || "La herramienta falló")), data: materialized?.data ?? null };
  ...
  for (let step = startStep; step < maxSteps; step += 1) {
    ...
    if (pending.length) {
      const workflow = workflowId
        ? { id: workflowId }
        : await createPendingWorkflow({ userId: options.userId, sessionId: options.sessionId, aiType: options.aiType, mode: options.mode, model, step, messages: currentMessages, pendingActions: pending, executedResults: results });
    ...
    if (!executable.length) return { response: appendUsedSources(text, usedSources), executedActions: executedActions.length ? executedActions : undefined };
  }
...
export async function resumeWorkflow(workflowId: string, tool: string, args: Record<string, any>) {
  ...
  const options: WorkflowRunOptions = { mode: workflow.mode, userId: workflow.user_id, sessionId: workflow.session_id, aiType: workflow.ai_type, maxSteps: MAX_STEPS, maxParallelTools: MAX_PARALLEL, workflowId, workflowMessages: Array.isArray(workflow.messages) ? workflow.messages : [] };
  ...
  if (!approved.success) { await finishWorkflow(workflowId, "error", { pending_actions: [], error: approved.message }); return { response: "", error: approved.message }; }
