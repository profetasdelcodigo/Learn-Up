...
function normalizeModel(value: unknown): { provider: Provider; model: string } {
  ...
  if (!raw || raw === "openrouter/free" || raw === "openrouter/openrouter/free") return { provider: "openrouter", model: "openrouter/free" };
...
function createProviderModel(selection: ReturnType<typeof normalizeModel>) {
  ...
  return createGoogleGenerativeAI({ apiKey: key })(selection.model);
}
...
export async function POST(req: Request) {
  try {
    ...
    const { messages, aiType, isAutonomous } = body as { messages: UIMessage[]; aiType?: string; isAutonomous?: boolean };
